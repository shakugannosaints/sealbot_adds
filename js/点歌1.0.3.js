  ...
  cmdQQMusic.solve = (ctx, msg, cmdArgs) => {
    ...
    switch (val) {
      ...
      default: {
        ...
        let url =
          "https://c.y.qq.com/soso/fcgi-bin/music_search_new_platform?searchid=53806572956004615&t=1&aggr=1&cr=1&catZhida=1&lossless=0&flag_qc=0&p=1&n=2&w=" + encodeURIComponent(musicName);
        // 发送 GET 请求
        fetch(url)
          .then((response) => {
            // 判断响应状态码是否为 200
  ...
  cmdCloudMusic.solve = (ctx, msg, cmdArgs) => {
    ...
    switch (val) {
      ...
      default: {
        ...
        let musicName = encodeURIComponent(val);
        let url =
          "https://cloudmusic.aivu.top/cloudsearch?keywords=" + (musicName);
        // 发送 GET 请求
        fetch(url)
          .then((response) => {
            // 判断响应状态码是否为 200
